...
$pageTitle = 'Instagram - Tagged';
$activePage = 'profile';
$currentUserId = (int) ($_SESSION['user_id'] ?? 0);
$profileUserId = (int) ($_GET['id'] ?? $currentUserId);
$posts = (new PostModel())->getTaggedPosts($profileUserId);
$profile = (new UserModel())->getProfile($profileUserId, $currentUserId);
...
<div class="post_page d-flex">
    ...
    <main class="flex-grow-1 py-4 px-3" style="max-width:935px;margin:auto">
        <div class="d-flex flex-column flex-md-row gap-5 align-items-center align-items-md-start mb-5">
            ...
            <div class="flex-grow-1">
                <div class="d-flex flex-wrap align-items-center gap-3 mb-3">
                    ...
                    <?php else: ?>
                        <a href="<?= htmlspecialchars(app_url('views/messages.php?user=' . $profileUserId)) ?>" class="btn btn-outline-secondary btn-sm">Message</a>
                    <?php endif; ?>
                </div>
                <div class="d-flex gap-4 mb-3">
                    <span><strong><?= (int) ($profile['post_count'] ?? 0) ?></strong> posts</span>
                    <span><strong><?= number_format($followModel->getFollowersCount($profileUserId)) ?></strong> followers</span>
                    <span><strong><?= number_format($followModel->getFollowingCount($profileUserId)) ?></strong> following</span>
                </div>
        ...
        <ul class="nav border-top justify-content-center gap-4 mb-4">
            <li class="nav-item"><a class="nav-link text-muted d-flex align-items-center gap-1" href="<?= htmlspecialchars(app_url('views/profile.php?id=' . $profileUserId . '&tab=posts')) ?>"><i class="fa-solid fa-table-cells-large"></i> POSTS</a></li>
            <li class="nav-item"><a class="nav-link text-muted d-flex align-items-center gap-1" href="<?= htmlspecialchars(app_url('views/profile.php?id=' . $profileUserId . '&tab=reels')) ?>"><i class="fa-solid fa-video"></i> REELS</a></li>
            <?php if($profileUserId === $currentUserId): ?>
                <li class="nav-item"><a class="nav-link text-muted d-flex align-items-center gap-1" href="<?= htmlspecialchars(app_url('views/saved.php')) ?>"><i class="fa-regular fa-bookmark"></i> SAVED</a></li>
            <?php endif; ?>
            <li class="nav-item"><a class="nav-link active text-dark fw-semibold d-flex align-items-center gap-1" href="<?= htmlspecialchars(app_url('views/tagged.php?id=' . $profileUserId)) ?>"><i class="fa-solid fa-tag"></i> TAGGED</a></li>
        </ul>
        ...
        <?php else: ?>
            <?php render_empty_state('fa-solid fa-tag', 'No tagged posts', 'Posts with @username mentions will show up here.'); ?>
        <?php endif; ?>
    </main>
</div>
<style>
